...
const Contador = ({ combate, setCombate, setJugadas }) => {
    const [tiempo, setTiempo] = useState(combate.segundos || 0); // Inicializa con carrera.segundos
    ...
    useEffect(() => {
        if (tiempo === 0) {
            ...
            const timeout = setTimeout(() => {
                evento(setCombate); // Llama a la función `evento` con `setCarrera` como parámetro
    ...
    return (
        <div>
            <div className="fs-4">
                Combate#: <span className="fw-bold">{combate.idevento}</span> inicia en <span className="fw-bold">{formatearTiempo(tiempo)}</span>
            </div>
