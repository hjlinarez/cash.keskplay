...
const Contador = ({ carrera, setCarrera, setJugadas }) => {
    const [tiempo, setTiempo] = useState(carrera.segundos || 0); // Inicializa con carrera.segundos
    ...
    useEffect(() => {
        if (tiempo === 0) {
            ...
            const timeout = setTimeout(() => {
                evento(setCarrera); // Llama a la función `evento` con `setCarrera` como parámetro
    ...
    return (
        <div>
            <div className="fs-4">
                Carrera#: <span className="fw-bold">{carrera.idevento}</span> inicia en <span className="fw-bold">{formatearTiempo(tiempo)}</span>
            </div>
